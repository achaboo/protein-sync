...
- **運動の「時刻」は入力しない**（日に何回か運動するため）。③の見出しも「今日の運動・食事の時刻」。
  ...
  2回目は夕食の3時間後なので時刻が出る
- **運動は曜日で固定しない。その日の実績を③に入れる。**「8種目×3セット」を毎日の標準にしない。
  有酸素はプリセット（なし／ウォーキング10km／サイクリング20km）、筋トレは部位分けの
  プリセット（全種目／背中・腕／胸・肩／胸・腕・肩（後）／なし）。消費エネルギーに入るのは
  チェックの入っている種目だけ。**「運動なし」と「未入力」を区別する**（`exDate`）
