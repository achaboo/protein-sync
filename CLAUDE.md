...
- **必要エネルギーは推定値**。`BMR × 1.50 ＋ 入力した運動の正味消費`（METs−1ベース。
  筋トレは0.032/総負荷量kg、ウォーキング0.5、サイクリング0.28）。
  これは**このツール独自の推定モデル**であって「医学的に正しい消費エネルギーの求め方」ではない。
  1.50は座位中心の生活を想定した値で、個人の日常活動量を表すものではないと明記する。
...
- **基本係数（1.65 g/kg）と活動レベル（`ACTIVITY` 1.50）は別物**。前者はたんぱく質、後者はエネルギー。
